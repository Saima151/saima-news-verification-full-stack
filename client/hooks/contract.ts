...
export const CONTRACT_ADDRESS =
  "CAIOXFFCWVHJEJ5H5YQNY77GFDDLDSLI744BT3CTYWXEMWWT6VKLIF4K";
 ...
 * Calls: submit_news(user: Address, headline: String, source: String, content_hash: String) -> u32
 */
export async function submitNews(
  ...
  return callContract(
    "submit_news",
    [toScValAddress(caller), toScValString(headline), toScValString(source), toScValString(contentHash)],
    caller,
    true
  );
}
...
/**
 * Verify submitted news.
 * Calls: verify_news(verifier: Address, news_id: u32, status: String)
 */
export async function verifyNews(
  ...
  return callContract(
    "verify_news",
    [toScValAddress(caller), toScValU32(newsId), toScValString(status)],
    caller,
    true
  );
}
...
/**
 * Get news details by ID (read-only).
 * Calls: get_news(news_id: u32) -> News
 */
...
/**
 * Get total news count (read-only).
 * Calls: get_news_count() -> u32
 */
export async function getNewsCount(caller?: string) {
  return readContract(
    "get_news_count",
    [],
    caller
